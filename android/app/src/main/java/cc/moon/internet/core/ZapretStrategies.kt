 ...
 *
 * And every line has since been run end to end — a real TLS handshake through byedpi to a
 * blocked host — not merely started. That caught the opposite failure: `-f-1` puts the fake
 * ahead of the whole payload and breaks the handshake outright, at any TTL. Six of these were
 * written that way. It is only safe next to `-A`, where the fake is held back until something
 * has actually failed; on its own the position must be 1 or 2.
 */
object ZapretStrategies {
    ...
    val all: List<ZapretStrategy> = listOf(
        ...
        s("general (FAKE TLS AUTO ALT)",     "-f1", "-n", FAKE_SNI, "-At,r"),
        s("general (FAKE TLS AUTO ALT2)",    "-f-1", "-n", FAKE_SNI, "-As"),
        s("general (FAKE TLS AUTO ALT3)",    "-f2", "-n", FAKE_SNI, "-r1+s", "-Kt,h"),
    )
